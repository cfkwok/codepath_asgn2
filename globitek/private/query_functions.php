<?php
  ...
  function validate_state($state, $errors=array()) {
    ...
    } elseif (!has_valid_place_name($state['name'])) {
      $errors[] = "State name may only contain letters A-Z, a-z, and -.";
    }

    if (is_blank($state['code'])) {
      $errors[] = "State code cannot be blank.";
    } elseif (!has_length($state['code'], array('min' => 2, 'max' => 4))) {
      $errors[] = "State code must be between 2 and 4 characters.";
    } elseif (!has_valid_state_code($state['code'])) {
      $errors[] = "State code may only contain alphabet letters.";
    }
  ...
  function validate_territory($territory, $errors=array()) {
    ...
    } elseif (!has_valid_place_name($territory['name'])) {
      $errors[] = "Territory name may only contain alphabet letters, -, and spaces.";
    }

    if (is_blank($territory['position'])) {
      $errors[] = "Territory position cannot be blank.";
    } elseif (!is_number($territory['position'])) {
      $errors[] = "Territory position should be a number.";
    }
  ...
  function insert_territory($territory) {
    ...
    $sql .= "(name, state_id, position) VALUES (";
    $sql .= "'" . $territory['name'] . "', ";
  ...
  function validate_salesperson($salesperson, $errors=array()) {
    ...
    if (is_blank($salesperson['phone'])) {
      ...
      $errors[] = "Invalid characters for phone number. Can only have 0-9 and ()-.";
    } 
  ...
  function validate_user($user, $errors=array()) {
    ...
    if (is_blank($user['email'])) {
      ...
      $errors[] = "Email must be a valid format (ie. johnsmith@gmail.com).";
    }
    ...
    if (is_blank($user['username'])) {
      ...
      $errors[] = "Username contains invalid characters. Allowed: A-Z, a-z, 0-9, and _.";
    }
    return $errors;
  }
